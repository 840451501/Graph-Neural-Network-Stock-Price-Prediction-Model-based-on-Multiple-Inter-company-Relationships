...
from model_pyg import get_loss, GATLSTM
import time
...
edge_index = torch.tensor(rel_encoding[:,:,0], dtype=torch.long)
edge_index = sp.csr_matrix(edge_index)
edge_index = from_scipy_sparse_matrix(edge_index)[0].cuda()

for i in range(0, 300, 200):#数据集开始时间
    model = GATLSTM(
        batch_size,
