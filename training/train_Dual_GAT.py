...
from model_pyg import get_loss, GATLSTM_multi_pro
import time
...
edge_index_list = []
edge_index = sp.csr_matrix(torch.tensor(rel_encoding[:,:,0], dtype=torch.long))
edge_index_list.append(from_scipy_sparse_matrix(edge_index)[0].cuda())

edge_index = sp.csr_matrix(torch.tensor(rel_encoding[:,:,1], dtype=torch.long))
edge_index_list.append(from_scipy_sparse_matrix(edge_index)[0].cuda())

edge_index = sp.csr_matrix(torch.tensor(rel_encoding[:,:,2], dtype=torch.long))
edge_index_list.append(from_scipy_sparse_matrix(edge_index)[0].cuda())

for i in range(0, 100, 100):#数据集开始时间
    model = GATLSTM_multi_pro(
        batch_size,
